...
from Source.model_func import load_model1, GradientDescent, load_model2, load_model3
...
def compute_folding_ratio(flow):
    """
    Compute 2D Jacobian folding ratio (batch average, returned in percent).
    Args:
        flow: Tensor, shape (B, 2, H, W), (dx, dy)
    Returns:
...
def main(path):
    ...
    seed_torch(cfg['Seed'])
    writer = SummaryWriter(os.path.join(cfg['CheckpointsPath'], 'Logs', 'SN', '001vh'))
    ...
    model2 = load_model2(cfg, model1=model1)

    model1_path = os.path.join(cfg['CheckpointsPath'], 'Weights', 'model_oa_low_best.pth.gz')
    model2_path = os.path.join(cfg['CheckpointsPath'], 'Weights', 'model_oa_mid_best.pth.gz')

    model1.load_weight(model1_path)
    model2.load_weight(model2_path)

    model = load_model3(cfg, model1=model1, model2=model2).to(device)
    ...
    for epoch in range(cfg['StartEpoch'], cfg['NumEpoch']+cfg['StartEpoch']):
        model.train()
        log_file = os.path.join(cfg['CheckpointsPath'], 'Results', 'train_history_bott.csv')
        ...
        if epoch % cfg['ValFreq'] == 0:
            ...
            if running_metric <= best_sim_metric:
                torch.save(model.state_dict(), os.path.join(cfg['CheckpointsPath'], 'Weights', 'model_sn_hi_best.pth.gz'))
        ...
        if cfg['EfficientSave']:
            torch.save(model.state_dict(), os.path.join(cfg['CheckpointsPath'], 'Weights', 'model_sn_hi_last.pth.gz'))
        else:
            torch.save(model.state_dict(), os.path.join(cfg['CheckpointsPath'], 'Weights', f'model_sn_hi_last_{epoch}.pth.gz'))
