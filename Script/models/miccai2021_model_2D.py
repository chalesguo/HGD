...
class Miccai2021_LDR_conditional_laplacian_unit_disp_add_lvl1(nn.Module):
    ...
    def forward(self, x, y, reg_code):
        ...
        fea_e0 = self.input_encoder_lvl1(cat_input_lvl1)  # feature
        e0 = self.down_conv(fea_e0)  # self.flow(feature)
        ...
        # print(output_disp_e0_v.permute(0, 2, 3, 1).shape)
        # print(self.grid_1.shape)

        warpped_inputx_lvl1_out = self.transform(down_x, output_disp_e0_v.permute(0, 2, 3, 1), self.grid_1)  # 问题来了  是不是流场的问题阿output_disp_e0_v
        # warp = warpped_inputx_lvl1_out.unsqueeze(-1).data.cpu().numpy()[0, 0, :, :, :]
        # warpped_inputx_lvl1_out = self.transform(x, output_disp_e0_v.permute(0, 2, 3, 1), self.grid_1)
        # cv2.imshow('tu', warp.squeeze(-1))
...
class Miccai2021_LDR_conditional_laplacian_unit_disp_add_lvl2(nn.Module):
    ...
    def forward(self, x, y, reg_code):
        ...
        e0 = self.up(e0)
        output_disp_e0_v = self.output_lvl1(torch.cat([e0, fea_e0], dim=1)) * self.range_flow
        compose_field_e0_lvl1 = lvl1_disp_up + output_disp_e0_v  # 流场 lv1+lv2
        warpped_inputx_lvl1_out = self.transform(x, compose_field_e0_lvl1.permute(0, 2, 3, 1), self.grid_1)  # 形变图像
...
class SpatialTransform_unit(nn.Module):
    ...
    def forward(self, x, flow, sample_grid):
        ...
        return flow
    # def forward(self, src, flow, sample_grid, mode='bilinear'):
    #     # new locations
