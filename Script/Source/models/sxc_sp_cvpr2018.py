...
class cvpr2018_net1(nn.Module):
    ...
    def __init__(self, vol_size, enc_nf, dec_nf, full_size=True):
        ...
        self.flow = conv_fn(dec_nf[-1], dim, kernel_size=3,
                            padding=1)  # 相当于Conv2d(8,2,3,1) x输入为torch.Size([1, 2, 192, 160]) 通过Unet网络变为torch.Size([1, 8, 192, 160])
        # 为了和后面的采样网格配合  通道为2  所以通过这个Conv2d(8,2,3,1) 将[1, 8, 192, 160] 变为[b,2,h,w]
        # Make flow weights + bias small. Not sure this is necessary.
        nd = Normal(0, 1e-5)
        self.flow.weight = nn.Parameter(nd.sample(self.flow.weight.shape))
        self.flow.bias = nn.Parameter(torch.zeros(
            self.flow.bias.shape))  # 这里的作用将flow层的weight和bias 初始化 使其比较小 在0-0.00001范围内  实际上nn.Parameter还有一个作用  自定义网络   就是将参数放入训练网络 可以被optim.step更新

        self.spatial_transform = SpatialTransformer3(vol_size)  # vol_size 图片的尺寸大小  这里指192,160
    ...
    def forward(self, src, tgt, iir_iter):
        ...
        flow1 = torch.zeros(b_size, 2, h_x1, w_x1, dtype=init_dtype, device=init_device).float()  # 定义变量 h_x1 w_x1就是改变图片大小
        src_move = src

        # for j in range(iir_iter):
        cat_input = torch.cat((src, tgt), 1)
        # cat_input = self.down_avg(cat_input)
        # cat_input_lvl1 = self.down_avg(cat_input)  三级使用
        ...
        for j in range(iir_iter):
            ...
            flow = flow + flow_fine   # flow是流场，也称为形变场
            y = self.spatial_transform(down_x, flow)  # 经过空间变换后的插值图像吧！ 记得改
            down_x = y
        ...
        flow1 = self.down_avg(flow)  # 降维？提取流场的关键特征？
        fwd = {'Moving': down_x, 'Fixed': down_y, 'Flow': flow, 'Moved': down_x}
        return fwd
        # return down_x, flow, down_y, flow1  # 插值图像 流场 不知道啥（可能是fix图） 关键流场
...
class cvpr2018_net2(nn.Module):
    ...
    def __init__(self, vol_size, enc_nf, dec_nf, full_size=True, model1=None):
        ...
        self.spatial_transform = SpatialTransformer2(vol_size)  # vol_size 图片的尺寸大小  这里指192,160
...
class SpatialTransformer1(nn.Module):
    ...
    def __init__(self, size, mode='bilinear'):
        ...
        vectors = [torch.arange(0, s / 4) for s in size]  # x为 0-191  y为0-159
        grids = torch.meshgrid(vectors)  # 生成网格 坐标矩阵  包含 两个二维数组（y,x）格式      和y坐标矩阵（H 192） x坐标矩阵（W 160）
        grid = torch.stack(grids)  # y, x, z  #将x坐标矩阵 和y坐标矩阵进行合并 并增加一个新的维度  由x:[192 160]  y:[192 160] 合并为[2,192,160]
        grid = torch.unsqueeze(grid, 0)  # add batch  在添加一个维度 [2,192,160] --[1,2,192,160]
        grid = grid.type(torch.FloatTensor)
        self.register_buffer('grid',
                             grid)  # grid看做是一个常量    反向传播不需要被optimizer更新，称之为 buffer   创建这个变量  不需要进行反向更新     def register_buffer(self, name, tensor):
        # grid requires_grad=false                                                               #      self._buffers[name] = tensor
    ...
    def forward(self, down_x, flow):
        ...
        """
        new_locs = self.grid + flow  # 规则网格加上flow 变成采样网格  shape   [1, 2, 192, 160])

        shape = flow.shape[2:]  # shape [192, 160]
        # Need to normalize grid values to [-1, 1] for resampler   将new_locs进行归一化
        for i in range(len(shape)):
            new_locs[:, i, ...] = 2 * (new_locs[:, i, ...] / (shape[
                                                                  i] - 1) - 0.5)  # x的范围为0-191  shape[0]为192 然后除以191 在乘于2  范围变为0-2   grid的范围应该为-1-1  所以 减去1 变为-1-1
        if len(shape) == 2:
            new_locs = new_locs.permute(0, 2, 3, 1)  # 将[N,C H,W] BI变为[N,H,W,c]
            new_locs = new_locs[..., [1, 0]]  # 将两列交换了一下  左右交换   为什么要交换？  前面[b,2 h,w] 是（y，x）坐标格式  现在要换成(x,y)格式
...
class SpatialTransformer2(nn.Module):
    ...
    def __init__(self, size, mode='bilinear'):
        ...
        # Create sampling grid
        vectors = [torch.arange(0, s / 2) for s in size]  # x为 0-191  y为0-159
        grids = torch.meshgrid(vectors)  # 生成网格 坐标矩阵  包含 两个二维数组（y,x）格式      和y坐标矩阵（H 192） x坐标矩阵（W 160）
        grid = torch.stack(grids)  # y, x, z  #将x坐标矩阵 和y坐标矩阵进行合并 并增加一个新的维度  由x:[192 160]  y:[192 160] 合并为[2,192,160]
        grid = torch.unsqueeze(grid, 0)  # add batch  在添加一个维度 [2,192,160] --[1,2,192,160]
        grid = grid.type(torch.FloatTensor)
...
class SpatialTransformer3(nn.Module):
    ...
    def __init__(self, size, mode='bilinear'):
        ...
        # Create sampling grid
        vectors = [torch.arange(0, s) for s in size]  # x为 0-191  y为0-159
        grids = torch.meshgrid(vectors)  # 生成网格 坐标矩阵  包含 两个二维数组（y,x）格式      和y坐标矩阵（H 192） x坐标矩阵（W 160）
        grid = torch.stack(grids)  # y, x, z  #将x坐标矩阵 和y坐标矩阵进行合并 并增加一个新的维度  由x:[192 160]  y:[192 160] 合并为[2,192,160]
        grid = torch.unsqueeze(grid, 0)  # add batch  在添加一个维度 [2,192,160] --[1,2,192,160]
        grid = grid.type(torch.FloatTensor)
