...
def spatial_grads_to_sampling_grid_2d(spatial_grads: torch.Tensor):
    ...
    生成的采样网格与`torch.grid_sample`函数所期望的输入兼容。

    参数:
    spatial_grads (torch.Tensor): 2D空间梯度张量，包含沿y和x轴的梯度信息。

    返回:
    torch.Tensor: 生成的2D采样网格。

    注意:
    输入张量的形状应为 (batch_size, 2, height, width)。
    """
    # 获取输入张量的维度信息
...
# reduction_ratio = 8
#
# se_module = SqueezeExcitation2d(channel_size, reduction=reduction_ratio)
#
# # 假设输入张量的形状为 (batch_size, channel_size, height, width)
# input_tensor = torch.randn((2, channel_size, 32, 32))
#
