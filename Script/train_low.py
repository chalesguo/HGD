...
from Source.model_func import load_model1, GradientDescent
...
def compute_folding_ratio(flow):
    ...
    kx = torch.tensor([[[[-0.5, 0.0, 0.5]]]], dtype=dtype, device=device)  # 1x1x1x3
    ky = kx.permute(0, 1, 3, 2)  # 1x1x3x1
...
def main(path):
    ...
    seed_torch(cfg['Seed'])
    writer = SummaryWriter(os.path.join(cfg['CheckpointsPath'], 'Logs', 'OA', '01l'))
    ...
    model = load_model1(cfg).to(device)
    ...
    for epoch in range(cfg['StartEpoch'], cfg['NumEpoch']+cfg['StartEpoch']):
        model.train()
        log_file = os.path.join(cfg['CheckpointsPath'], 'Logs', 'train_history.csv')
        ...
        if cfg['EfficientSave']:
            ...
            torch.save(model.state_dict(), os.path.join(cfg['CheckpointsPath'], 'Weights', f'model_last_{epoch}.pth.gz'))
