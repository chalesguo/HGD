...
def main(path):
    ...
    model2 = load_model2(cfg, model1=model1).to(device)
    model = load_model3(cfg, model1=model1, model2=model2).to(device)  

    model_path = os.path.join(cfg['CheckpointsPath'], 'Weights', 'MLDB', 'bzd', 'model_ml_hi_best.pth.gz')
    ...
    for batch_idx, (subj_id) in enumerate(test_loader):
        ...
        # abs_diff = cv2.absdiff(fixed, moved_img)
        # plt.axis('off')
        # plt.imshow(abs_diff, cmap='hot')
        # plt.savefig(os.path.join(slice_save_path, 'ML', 'ABS', 'Rbs.png'))
        # plt.show()
        # cv2.imwrite(os.path.join(slice_save_path, 'ML', 'flow', 'flow_{}.png'.format(batch_idx)), flow)
        cv2.imwrite(os.path.join(slice_save_path, 'ML', 'sxc', 'Moved_{}.png'.format(batch_idx)), moved_img)
